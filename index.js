...
// Bepul Render tarifida CPU juda kam (0.1 ulush) bo'lgani uchun o'lcham/bitreytni
// pasaytirdik — bu ffmpeg siqish vaqtini sezilarli qisqartiradi. Ilovada video
// kartochka atigi ~132-150px kenglikda ko'rsatiladi, shuning uchun sifat yetarli bo'ladi.
const MAX_DIM = 360;
const TARGET_VIDEO_BITRATE = '450k';
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          ...
          .outputOptions(['-preset ultrafast', '-r 20', '-movflags +faststart', '-c:v libx264', '-c:a aac', '-pix_fmt yuv420p'])
          .on('error', reject)
          .on('end', resolve)
          .save(outputPath);
      });
